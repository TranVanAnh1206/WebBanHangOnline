...
namespace WebBanHangOnline.Controllers
{
    public class ShoppingCartController : Controller
    {
        ...
        {
            var code = new { Success = false, Code = -1, Message = "" };
        ...
        [HttpPost]
        public ActionResult AddToCart(int id, int quantity, string prodSizeId, string prodColorId)
        {
            ...
            {
                // Kiểm tra và chuyển đổi trước khi sử dụng trong truy vấn LINQ
                int sizeId, colorId;
                bool isSizeValid = int.TryParse(prodSizeId, out sizeId);
                bool isColorValid = int.TryParse(prodColorId, out colorId);

                var size = isSizeValid ? db.Sizes.FirstOrDefault(x => x.Id == sizeId && x.ProductId == id) : null;
                var color = isColorValid ? db.Colors.FirstOrDefault(x => x.Id == colorId && x.ProductId == id) : null;
                ...
                ShoppingCartItem item = new ShoppingCartItem
                {
                    ...
                    ProductColorId = color == null ? 0 : color.Id,
                    ProductColorName = color == null ? "" : color.ProdColor,
                    ProductSizeName = size == null ? "" : size.ProdSize,
                    ProductSizeId = size == null ? 0 : size.Id,
        ...
        [HttpPost]
        public ActionResult Delete(int id, int prodSize, int prodColor)
        {
            var code = new { Success = false, msg = "", code = -1, Count = 0 };
            ...
            if (cart != null)
            {
                ...
                if (checkProduct != null)
                {
                    cart.Remove(id, prodSize, prodColor);
